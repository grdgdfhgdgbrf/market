...
class OrderStatus(Enum):
    ...
    PAID = "paid"  # Оплачено звездами
    CONFIRMED = "confirmed"  # Подтверждено админом (для рублей)
    CANCELLED = "cancelled"  # Отменено
    COMPLETED = "completed"  # Выполнено продавцом
...
class Order:
    def __init__(self, order_id: str, user_id: int, username: str, items: Dict[str, int], 
        ...
        self.completed_by = None  # ID продавца, который завершил заказ
...
class Database:
    def __init__(self):
        ...
        self.carts = {}  # user_id -> UserCart
        self.promocodes = {}  # code -> Promotion
        self.support_tickets = {}  # ticket_id -> SupportTicket
        ...
        self.bot_username = "ShopBot"  # Имя продавца/магазина
    ...
    def get_active_orders(self) -> List[Order]:
        """Заказы, ожидающие выполнения продавцом"""
    ...
    def get_user_tickets(self, user_id: int) -> List[SupportTicket]:
        return [t for t in self.support_tickets.values() if t.user_id == user_id]
...
def get_product_inline_keyboard(product_id: str, current_qty: int = 1):
    ...
    # Кнопка для ручного ввода количества
    builder.row(
...
class UserStates(StatesGroup):
    ...
    waiting_for_manual_quantity = State()  # Новое состояние для ручного ввода количества
...
async def process_manual_quantity(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Показываем товар с выбранным количеством
        rub_price = f"{product.price_rub:.2f}".rstrip('0').rstrip('.') if product.price_rub % 1 else str(int(product.price_rub))
        stars_price = f"{product.price_stars:.2f}".rstrip('0').rstrip('.') if product.price_stars % 1 else str(int(product.price_stars))
...
async def add_product_description(message: Message, state: FSMContext):
    ...
    await message.answer("Введите цену в рублях (можно использовать дробные числа, например 99.99):")
...
async def add_product_price_rub(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("Введите цену в звездах (можно использовать дробные числа, например 9.99):")
    except ValueError:
        await message.answer("❌ Введите корректное число (можно дробное)")
...
async def edit_product_start(message: Message, state: FSMContext):
    ...
    text = "Выберите ID товара для редактирования:\n\n"
    for product in products:
        rub_price = f"{product.price_rub:.2f}".rstrip('0').rstrip('.') if product.price_rub % 1 else str(int(product.price_rub))
        stars_price = f"{product.price_stars:.2f}".rstrip('0').rstrip('.') if product.price_stars % 1 else str(int(product.price_stars))
        text += f"ID: {product.id} - {product.name} ({rub_price}₽ / {stars_price}⭐)\n"
...
async def edit_product_select(message: Message, state: FSMContext):
    ...
    text = (
        ...
        f"1 - Название (сейчас: {product.name})\n"
        f"2 - Описание (сейчас: {product.description})\n"
        f"3 - Цена в рублях (сейчас: {rub_price})\n"
        f"4 - Цена в звездах (сейчас: {stars_price})\n"
        f"5 - Количество (сейчас: {product.stock})\n\n"
        f"Введите номер поля (1-5):"
    )
    
    await message.answer(text)

@dp.message(AdminStates.waiting_for_product_edit_field)
async def edit_product_field(message: Message, state: FSMContext):
    ...
    try:
        ...
        prompts = {
            ...
            "price_rub": "Введите новую цену в рублях (можно дробную):",
            "price_stars": "Введите новую цену в звездах (можно дробную):",
            "stock": "Введите новое количество (целое число):"
        }
...
async def edit_product_value(message: Message, state: FSMContext):
    ...
    try:
        if field in ["price_rub", "price_stars"]:
        ...
        if (field in ["price_rub", "price_stars", "stock"] and value < 0):
            raise ValueError
        
        product = db.update_product(product_id, **{field: value})
...
async def delete_product_confirm(message: Message, state: FSMContext):
    ...
    if db.delete_product(product_id):
        await message.answer(
            f"✅ Товар '{product.name}' (ID: {product_id}) удален.",
            reply_markup=get_admin_keyboard()
        )
    else:
...
async def show_all_orders(message: Message):
    ...
    for order in orders[:10]:
        ...
        if order.receipt_sent:
            text += f"🧾 Чек отправлен: ✅\n"
        
        if order.completed_by:
            text += f"👨‍💼 Завершил: Admin ID {order.completed_by}\n"
...
async def show_pending_orders(message: Message):
    ...
    pending_orders = [o for o in pending_orders if o.payment_method == PaymentMethod.RUBLES]
...
@dp.message(F.text == "🔄 Активные заказы")
async def show_active_orders_admin(message: Message):
    if not is_admin(message.from_user.id) or is_group_chat(message):
        return
    ...
    if not active_orders:
        await message.answer("✅ Нет активных заказов (оплаченных и ожидающих выполнения).")
        return
    ...
    for order in active_orders:
        ...
        text = (
            f"🔄 *Активный заказ #{order.order_id}*\n"
            f"👤 Пользователь: @{order.username} (ID: {order.user_id})\n"
            f"📅 Создан: {order.created_at.strftime('%d.%m.%Y %H:%M')}\n"
            f"💳 Оплата: {'⭐ Звезды' if order.payment_method == PaymentMethod.STARS else '💳 Рубли'}\n"
            f"💰 Сумма: {total_rub_str}₽ / {total_stars_str}⭐\n"
            f"📦 Товары:\n{items_text}\n"
            f"📊 Статус: {order.status.value}\n"
        )
...
@dp.message(F.text == "✅ Завершенные заказы")
async def show_completed_orders_admin(message: Message):
    if not is_admin(message.from_user.id) or is_group_chat(message):
        return
    ...
    for order in completed_orders[:10]:
        ...
        text = (
            f"🎉 *Завершенный заказ #{order.order_id}*\n"
            f"👤 Пользователь: @{order.username} (ID: {order.user_id})\n"
            f"📅 Создан: {order.created_at.strftime('%d.%m.%Y %H:%M')}\n"
            f"✅ Завершен: {order.completed_at.strftime('%d.%m.%Y %H:%M') if order.completed_at else 'Неизвестно'}\n"
            f"👨‍💼 Завершил: Admin ID {order.completed_by if order.completed_by else 'Неизвестно'}\n"
            f"💳 Оплата: {'⭐ Звезды' if order.payment_method == PaymentMethod.STARS else '💳 Рубли'}\n"
            f"💰 Сумма: {total_rub_str}₽ / {total_stars_str}⭐\n"
            f"📦 Товары:\n{items_text}\n"
        )
    ...
    if len(completed_orders) > 10:
        await message.answer(f"Показано 10 из {len(completed_orders)} завершенных заказов")
...
async def admin_confirm_order(callback: CallbackQuery):
    ...
    if order.payment_method == PaymentMethod.RUBLES:
    ...
    receipt += f"\nСпасибо за покупку!\n"
    receipt += f"Вы можете посмотреть чек в разделе '🧾 Чеки'."
    ...
    try:
        ...
        pass
    await callback.message.reply(f"✅ Заказ #{order_id} подтвержден, чек отправлен пользователю")
    await callback.answer()
...
async def admin_complete_order(callback: CallbackQuery):
    ...
    completion_text = (
        ...
        f"📦 Товары:\n{items_text}\n"
        f"👨‍💼 Продавец: {db.get_bot_username()}\n\n"
        f"Вы можете оставить отзыв в разделе '⭐ Отзывы'."
    )
    ...
    try:
        ...
        pass
    await callback.message.reply(f"🎉 Заказ #{order_id} отмечен как выполненный")
    await callback.answer()
...
async def admin_cancel_order(callback: CallbackQuery):
    ...
    try:
        ...
        await bot.send_message(
            ...
            f"Сумма: {total_rub_str}₽\n"
            f"Если вы оплатили заказ, свяжитесь с поддержкой для возврата средств."
        )
    except Exception as e:
        logger.error(f"Failed to notify user {order.user_id}: {e}")
    ...
    try:
        ...
        pass
    await callback.message.reply(f"❌ Заказ #{order_id} отменен")
    await callback.answer()

@dp.callback_query(F.data.startswith("view_screen_"))
async def view_order_screenshot(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("Нет прав", show_alert=True)
        return
...
async def create_promocode_code(message: Message, state: FSMContext):
    ...
    await message.answer("Введите размер скидки в процентах (целое число):")
...
async def show_payment_details(message: Message):
    if not is_admin(message.from_user.id) or is_group_chat(message):
        return
    
    details = db.get_payment_details()
...
async def edit_card_value(message: Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await state.clear()
        await message.answer(
            "Действие отменено.",
...
async def edit_phone_value(message: Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await state.clear()
        await message.answer(
            "Действие отменено.",
...
async def edit_bank_value(message: Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await state.clear()
        await message.answer(
            "Действие отменено.",
    ...
    await message.answer(
        "✅ Название банка обновлено!",
        reply_markup=get_admin_keyboard()
    )
    await state.clear()

# ===== Система ответов поддержки =====
...
async def support_tickets_menu(message: Message):
    ...
    tickets = db.get_all_tickets(answered=False)
    
    if not tickets:
        await message.answer(
            "📨 Нет новых обращений в поддержку.",
            reply_markup=InlineKeyboardMarkup(
                inline_keyboard=[
                    [InlineKeyboardButton(text="📨 Показать все", callback_data="tickets_all")],
                    [InlineKeyboardButton(text="◀️ Назад", callback_data="back_to_admin")]
                ]
            )
        )
    ...
    text = f"📨 *Новые обращения в поддержку*\n\nВсего: {len(tickets)}"
    
    await message.answer(
        text,
        parse_mode="Markdown",
        reply_markup=get_support_tickets_inline_keyboard(0, False)
    )
...
async def show_answered_tickets(callback: CallbackQuery):
    try:
        await callback.message.edit_text(
            ...
            parse_mode="Markdown",
            reply_markup=get_support_tickets_inline_keyboard(0, True)
        )
    except TelegramBadRequest:
        await callback.message.delete()
        await callback.message.answer(
...
@dp.callback_query(F.data == "tickets_all")
async def show_all_tickets(callback: CallbackQuery):
    try:
        await callback.message.edit_text(
            "📨 *Все обращения:*",
            parse_mode="Markdown",
            reply_markup=get_support_tickets_inline_keyboard(0, None)
        )
    except TelegramBadRequest:
        await callback.message.delete()
        await callback.message.answer(
...
async def view_ticket(callback: CallbackQuery):
    ...
    status = "✅ Отвечен" if ticket.answered else "⏳ Ожидает ответа"
    ...
    if ticket.replies:
        text += "\n*История ответов:*\n"
        for i, reply in enumerate(ticket.replies, 1):
            text += f"\n{i}. {reply['text']}\n   (от {reply['created_at'].strftime('%d.%m.%Y %H:%M')})"
    
    if ticket.answered and ticket.answer_message:
        text += f"\n\n*Последний ответ:*\n{ticket.answer_message}\n"
        text += f"📅 Отвечен: {ticket.answered_at.strftime('%d.%m.%Y %H:%M')}\n"
    
    keyboard = InlineKeyboardBuilder()
    
    if not ticket.answered:
        keyboard.row(InlineKeyboardButton(text="✏️ Ответить", callback_data=f"answer_ticket_{ticket.ticket_id}"))
    else:
        keyboard.row(InlineKeyboardButton(text="✏️ Дополнительный ответ", callback_data=f"reply_ticket_{ticket.ticket_id}"))
    
    keyboard.row(
        InlineKeyboardButton(text="📨 Все тикеты", callback_data="tickets_pending"),
        InlineKeyboardButton(text="◀️ Назад", callback_data="back_to_admin")
    )
...
async def answer_ticket_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"✏️ Введите ответ пользователю @{ticket.username} по тикету #{ticket_id}:",
        reply_markup=get_cancel_keyboard()
    )
    await callback.answer()
...
async def reply_ticket_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"✏️ Введите дополнительный ответ пользователю @{ticket.username} по тикету #{ticket_id}:",
        reply_markup=get_cancel_keyboard()
    )
    await callback.answer()
...
async def process_ticket_answer(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(
            ...
            f"{message.text}\n\n"
            f"Вы можете ответить на это сообщение, и ваш ответ снова попадет в поддержку.",
            parse_mode="Markdown"
        )
        
        await message.answer(
            f"✅ Ответ отправлен пользователю @{ticket.username}",
            reply_markup=get_admin_keyboard()
        )
    except Exception as e:
...
async def process_ticket_reply(message: Message, state: FSMContext):
    ...
    try:
        await bot.send_message(
            user_id,
            f"📨 *Дополнительный ответ от поддержки*\n\n"
            f"По вашему обращению #{ticket_id}:\n\n"
            f"{message.text}",
            parse_mode="Markdown"
        )
        ...
        await message.answer(
            f"✅ Дополнительный ответ отправлен пользователю @{ticket.username}",
            reply_markup=get_admin_keyboard()
        )
    except Exception as e:
...
async def handle_user_reply_to_support(message: Message):
    if is_group_chat(message):
        return
    ...
    if "Ответ от поддержки" in reply_text or "Дополнительный ответ" in reply_text:
        match = re.search(r'#(T\d+)', reply_text)
        if match:
            ...
            if ticket:
                ...
                new_ticket = db.create_ticket(
                    user_id,
                    username,
                    f"[Ответ на #{ticket_id}] {message.text}",
                    message.message_id
                )
                ...
                for admin_id in ADMIN_IDS:
                    try:
                        ...
                        await bot.send_message(
                            ...
                            f"Тикет: #{ticket_id}\n"
                            f"Новый тикет: #{new_ticket.ticket_id}",
                            parse_mode="Markdown"
                        )
                    except:
                        pass
                
                await message.reply("✅ Ваш ответ передан в поддержку. Мы ответим вам в ближайшее время.")
...
async def show_statistics(message: Message):
    ...
    pending_orders = sum(1 for o in orders if o.status == OrderStatus.PENDING and o.payment_method == PaymentMethod.RUBLES)
    active_orders = sum(1 for o in orders if o.status in [OrderStatus.PAID, OrderStatus.CONFIRMED])
    cancelled_orders = sum(1 for o in orders if o.status == OrderStatus.CANCELLED)
    
    total_revenue_rub = sum(o.total_rub for o in orders if o.status == OrderStatus.COMPLETED)
    total_revenue_stars = sum(o.total_stars for o in orders if o.status == OrderStatus.COMPLETED)
    
    promocodes = list(db.promocodes.values())
    active_promos = sum(1 for p in promocodes if p.is_active and datetime.now() <= p.valid_until)
    
    total_tickets = len(tickets)
    pending_tickets = sum(1 for t in tickets if not t.answered)
    
    total_reviews = len(reviews)
    avg_rating = sum(r.rating for r in reviews) / total_reviews if total_reviews > 0 else 0
    reviews_with_reply = sum(1 for r in reviews if r.admin_reply)
    
    revenue_rub_str = f"{total_revenue_rub:.2f}".rstrip('0').rstrip('.') if total_revenue_rub % 1 else str(int(total_revenue_rub))
    revenue_stars_str = f"{total_revenue_stars:.2f}".rstrip('0').rstrip('.') if total_revenue_stars % 1 else str(int(total_revenue_stars))
    ...
    text = (
        ...
        f"• Активные (в работе): {active_orders}\n"
        f"• Ожидают оплаты: {pending_orders}\n"
        f"• Отменено: {cancelled_orders}\n\n"
        f"*Выручка (только завершенные):*\n"
        f"• Рубли: {revenue_rub_str}₽\n"
        f"• Звезды: {revenue_stars_str}⭐\n\n"
        f"*Промокоды:*\n"
        f"• Всего создано: {len(promocodes)}\n"
        f"• Активно: {active_promos}\n\n"
        f"*Поддержка:*\n"
        f"• Всего обращений: {total_tickets}\n"
        f"• Ожидают ответа: {pending_tickets}\n\n"
        f"*Отзывы:*\n"
        f"• Всего отзывов: {total_reviews}\n"
        f"• Средняя оценка: {avg_rating:.1f}⭐\n"
        f"• Отвечено: {reviews_with_reply}"
    )
