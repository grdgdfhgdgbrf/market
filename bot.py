...
class Database:
    ...
    def validate_promocode(self, code: str) -> Tuple[bool, int, str]:
        ...
        if datetime.now() > promo.valid_until:
            return False, 0, "Срок действия истек"
    ...
    def get_active_orders(self) -> List[Order]:
        return [o for o in self.orders.values() if o.status in [OrderStatus.PAID, OrderStatus.CONFIRMED]]
...
def get_products_inline_keyboard(page: int = 0, items_per_page: int = 5):
    ...
    for product in page_products:
        ...
        builder.row(InlineKeyboardButton(
            text=f"{product.name} - {product.price_rub}₽ / {product.price_stars}⭐{stock_status}",
...
def get_product_inline_keyboard(product_id: str, current_qty: int = 1):
    ...
    builder.row(
        ...
        InlineKeyboardButton(text=f"{current_qty}", callback_data="ignore"),
        InlineKeyboardButton(text="➕", callback_data=f"inc_{product_id}")
    )
    
    # Кнопка ручного ввода
    builder.row(
        InlineKeyboardButton(text="✏️ Ввести количество вручную", callback_data=f"manual_{product_id}")
    )
...
def get_order_actions_inline_keyboard(order_id: str, is_admin: bool = False):
    ...
    if is_admin:
        ...
        if order and order.status in [OrderStatus.PAID, OrderStatus.CONFIRMED]:
            builder.row(InlineKeyboardButton(text="🎉 Завершить", callback_data=f"admin_complete_{order_id}"))
        if order and order.screenshot_file_id:
            builder.row(InlineKeyboardButton(text="📸 Скрин", callback_data=f"view_screen_{order_id}"))
    else:
        if order and order.status == OrderStatus.PENDING and order.payment_method == PaymentMethod.RUBLES:
            builder.row(InlineKeyboardButton(text="📸 Отправить скрин", callback_data=f"send_screen_{order_id}"))
    return builder.as_markup()
...
class UserStates(StatesGroup):
    ...
    waiting_for_manual_quantity = State()  # Новое состояние
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        "Здесь вы можете приобрести товары за звезды или рубли.\n"
...
async def show_catalog(message: Message):
    ...
    if not products:
        ...
        return
    await message.answer("🛍 Наш каталог:", reply_markup=get_products_inline_keyboard())
...
async def show_cart(message: Message):
    ...
    for product_id, quantity in cart.items.items():
        product = db.get_product(product_id)
        if product:
            cart_text += f"• *{product.name}* x{quantity}\n"
            cart_text += f"  {product.price_rub}₽ x{quantity} = {product.price_rub * quantity:.2f}₽\n"
            cart_text += f"  {product.price_stars}⭐ x{quantity} = {product.price_stars * quantity:.2f}⭐\n\n"
            total_rub += product.price_rub * quantity
            total_stars += product.price_stars * quantity
    ...
    cart_text += f"*Итого:* {total_rub:.2f}₽ / {total_stars:.2f}⭐"
    
    await message.answer(cart_text, parse_mode="Markdown", reply_markup=get_cart_inline_keyboard(user_id))
...
async def show_active_orders(message: Message):
    ...
    for order in active_orders:
        ...
        text = (
            f"{status_text} *Заказ #{order.order_id}*\n"
            f"📅 {order.created_at.strftime('%d.%m.%Y %H:%M')}\n"
            f"💰 Сумма: {order.total_rub:.2f}₽ / {order.total_stars:.2f}⭐\n"
        )
...
async def support_request(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📝 Опишите вашу проблему. Напишите сообщение.",
...
async def process_support_message(message: Message, state: FSMContext):
    ...
    ticket = db.create_ticket(user_id, username, message.text or "[Медиа]", message.message_id)
    
    for admin_id in ADMIN_IDS:
        try:
            await message.copy_to(admin_id, caption=f"📨 Тикет #{ticket.ticket_id} от @{username}")
        except Exception as e:
            logger.error(f"Failed to forward to admin {admin_id}: {e}")
    
    await message.answer(
        f"✅ Сообщение отправлено. Номер обращения: #{ticket.ticket_id}",
        reply_markup=get_main_keyboard(is_admin(user_id))
    )
    await state.clear()
...
async def show_receipt(callback: CallbackQuery):
    ...
    for product_id, quantity in order.items.items():
        ...
        if product:
            ...
            receipt_text += f"  {product.price_rub}₽ x{quantity} = {product.price_rub * quantity:.2f}₽\n"
            receipt_text += f"  {product.price_stars}⭐ x{quantity} = {product.price_stars * quantity:.2f}⭐\n"
    
    receipt_text += "\n" + "=" * 30 + "\n"
    receipt_text += f"💰 *ИТОГО:* {order.total_rub:.2f}₽ / {order.total_stars:.2f}⭐\n"
    receipt_text += "=" * 30 + "\n"
...
async def write_review_start(message: Message, state: FSMContext):
    ...
    await message.answer("Выберите заказ для отзыва:", reply_markup=builder.as_markup())
...
async def select_order_for_review(callback: CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    for i in range(1, 6):
        builder.row(InlineKeyboardButton(text="⭐" * i, callback_data=f"rating_{i}"))
    builder.row(InlineKeyboardButton(text="◀️ Отмена", callback_data="cancel_review"))
    
    await callback.message.edit_text(
        f"Оцените заказ #{order_id} от 1 до 5 звезд:",
...
async def skip_review_comment(callback: CallbackQuery, state: FSMContext):
    ...
    await save_review(callback.from_user, data, "", callback.message)
    await state.clear()
...
async def process_review_comment(message: Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await state.clear()
        await message.answer("Действие отменено.", reply_markup=get_reviews_keyboard())
        return
    
    data = await state.get_data()
    await save_review(message.from_user, data, message.text, message)
    await state.clear()

async def save_review(user, data, comment, message_obj):
    order_id = data.get('review_order_id')
    rating = data.get('review_rating')
    review = db.add_review(user.id, user.username or "NoUsername", order_id, rating, comment)
    ...
    if group_id:
        try:
            await bot.send_message(group_id, review_text, parse_mode="Markdown")
        except Exception as e:
            logger.error(f"Failed to send review to group: {e}")
    
    await message_obj.answer("✅ Спасибо за отзыв!", reply_markup=get_reviews_keyboard())
...
async def process_pagination(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_reply_markup(reply_markup=get_products_inline_keyboard(page))
...
async def show_product(callback: CallbackQuery):
    ...
    text = (
        ...
        f"💰 Цена: {product.price_rub}₽ / {product.price_stars}⭐\n"
...
async def back_to_catalog(callback: CallbackQuery):
    try:
        await callback.message.edit_text("🛍 Наш каталог:", reply_markup=get_products_inline_keyboard())
    except TelegramBadRequest:
        await callback.message.delete()
        await callback.message.answer("🛍 Наш каталог:", reply_markup=get_products_inline_keyboard())
    await callback.answer()
...
async def increase_quantity(callback: CallbackQuery):
    ...
    if current_qty < product.stock:
        ...
        new_markup = get_product_inline_keyboard(product_id, current_qty)
        ...
        text = (
            ...
            f"💰 Цена: {product.price_rub}₽ / {product.price_stars}⭐\n"
            f"📦 В наличии: {product.stock} шт.\n\n"
            f"Выберите количество: {current_qty}"
        )
...
async def manual_quantity_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(product_id=product_id, product_name=product.name, max_stock=product.stock)
    
    await callback.message.answer(
        f"Введите количество товара *{product.name}* (доступно: {product.stock} шт.):",
        parse_mode="Markdown",
        reply_markup=get_cancel_keyboard()
    )
    await callback.answer()
...
async def process_manual_quantity(message: Message, state: FSMContext):
    ...
    try:
        ...
        if quantity > max_stock:
            await message.answer(f"❌ Доступно только {max_stock} шт.")
            return
        ...
        text = (
            ...
            f"💰 Цена: {product.price_rub}₽ / {product.price_stars}⭐\n"
            f"📦 В наличии: {product.stock} шт.\n\n"
            f"Выберите количество: {quantity}"
        )
...
async def add_to_cart(callback: CallbackQuery):
    ...
    if product.stock < current_qty:
        await callback.answer(f"Недостаточно товара. В наличии: {product.stock}", show_alert=True)
        return
...
async def view_cart_from_catalog(callback: CallbackQuery):
    ...
    await callback.message.answer(cart_text, parse_mode="Markdown", reply_markup=get_cart_inline_keyboard(user_id))
    await callback.answer()
...
async def remove_from_cart(callback: CallbackQuery):
    ...
    try:
        await callback.message.edit_text(cart_text, parse_mode="Markdown", reply_markup=get_cart_inline_keyboard(user_id))
    except:
        pass
...
async def pay_with_stars(callback: CallbackQuery):
    ...
    order = db.create_order(user_id, callback.from_user.username or "NoUsername", cart, PaymentMethod.STARS)
    ...
    await callback.message.answer_invoice(
        ...
        description=f"Оплата {stars_amount} ⭐",
...
async def pay_with_rubles(callback: CallbackQuery):
    ...
    order = db.create_order(user_id, callback.from_user.username or "NoUsername", cart, PaymentMethod.RUBLES)
...
async def send_screenshot_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"📸 Отправьте скриншот оплаты для заказа #{order_id}",
        reply_markup=get_cancel_keyboard()
    )
    await callback.answer()
...
async def process_screenshot(message: Message, state: FSMContext):
    ...
    admin_text = f"📸 Скрин оплаты для заказа #{order_id} от @{order.username}"
...
async def successful_payment_handler(message: Message):
    ...
    if order:
        ...
        receipt = f"🧾 Заказ #{order_id} оплачен звездами! Спасибо!"
        await message.answer(receipt)
        
        for admin_id in ADMIN_IDS:
            try:
                await bot.send_message(admin_id, f"💰 Заказ #{order_id} оплачен звездами!")
            except:
                pass
...
async def set_bot_username_start(message: Message, state: FSMContext):
    ...
    await message.answer("Введите имя продавца:", reply_markup=get_cancel_keyboard())
...
async def set_bot_username_process(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ Имя установлено: {message.text}", reply_markup=get_admin_keyboard())
    await state.clear()
...
async def add_product_stock(message: Message, state: FSMContext):
    ...
    try:
        ...
        data = await state.get_data()
        product = db.add_product(data['name'], data['description'], data['price_rub'], data['price_stars'], stock)
        await message.answer(f"✅ Товар добавлен! ID: {product.id}", reply_markup=get_admin_keyboard())
        await state.clear()
    except ValueError:
        await message.answer("❌ Введите целое число")

# Остальные админ-обработчики можно оставить без изменений...
...
async def cancel_review_callback(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.delete()
    await callback.message.answer("⭐ Раздел отзывов", reply_markup=get_reviews_keyboard())
    await callback.answer()
...
async def on_startup():
    ...
    db.add_product("Тест 1", "Описание 1", 99.99, 9.99, 100)
    db.add_product("Тест 2", "Описание 2", 199.50, 19.50, 50)
